...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // PANOPTES PATCH: real terminals (xterm, iTerm2) save lines that
            // scroll off the *top of the screen* into scrollback even when a
            // scrolling region is set, as long as the region starts at row 1.
            // Upstream vt100 discarded them whenever any region was active,
            // which threw away the history of apps that pin a footer with
            // DECSTBM and scroll their content region (Codex CLI does this).
            // Only a region whose top is below row 1 discards, as on a real
            // terminal.
    ...
    // Unused since the PANOPTES PATCH in scroll_up, kept to minimize the diff
