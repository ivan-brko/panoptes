...
pub struct Tailer {
    kind: TranscriptKind,
    path: PathBuf,
    /// Bytes already consumed, including any held in `partial`
    offset: u64,
...
impl Tailer {
    ...
    ///
    /// Seeking to EOF is what stops a resumed session replaying its entire
    /// history as if it were happening now. The cost is that the usage display
    /// would start blank, so the tail is scanned backwards once for the most
    /// recent figures, which are returned to seed it.
    ...
    /// Follow a file from the beginning, for tests and one-shot reads
    ...
    pub fn poll(&mut self) -> (Vec<AgentEvent>, Vec<String>) {
        ...
        // Lossy on purpose: a multi-byte character split across the read
        // boundary must not discard the rest of the chunk.
