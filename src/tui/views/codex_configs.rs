//! Codex configurations view
//!
//! Displays and manages OpenAI Codex CLI configurations (accounts).
...
/// Render the Codex configurations view
#[allow(clippy::too_many_arguments)]
pub fn render_codex_configs(
    frame: &mut Frame,
    area: Rect,
    ...
    // Build header
    let breadcrumb = Breadcrumb::new().push("Codex Configs");
...
fn render_empty_state(frame: &mut Frame, area: Rect) {
    ...
    let message = vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            "Each configuration points to a different CODEX_HOME directory,",
            Style::default().fg(t.text_muted),
        )),
        Line::from(Span::styled(
            "allowing you to use multiple Codex accounts.",
            Style::default().fg(t.text_muted),
        )),
        Line::from(""),
        Line::from(Span::styled(
            "The default config (~/.codex) is used if you don't specify a path.",
            Style::default().fg(t.text_muted),
        )),
    ];
...
fn render_config_list(
    ...
    area: Rect,
    configs: &[&CodexConfig],
    default_id: Option<CodexConfigId>,
    selected_index: usize,
) {
    let t = theme();
    ...
    let items: Vec<ListItem> = configs
        ...
        .map(|(i, config)| {
            ...
            let content = Line::from(vec![
                ...
                Span::styled(
                    format!("  {}", config.codex_home_display()),
                    Style::default().fg(t.text_muted),
                ),
            ]);
    ...
    let list = List::new(items).block(
        Block::default()
            .borders(Borders::ALL)
            .title(format!("Codex Configurations ({})", configs.len())),
    );
    frame.render_widget(list, area);
}

/// Render the Codex config name input dialog
pub fn render_codex_config_name_input_dialog(frame: &mut Frame, area: Rect, input: &str) {
    let t = theme();
...
/// Render the Codex config path input dialog
pub fn render_codex_config_path_input_dialog(
    frame: &mut Frame,
    area: Rect,
    name: &str,
    input: &str,
    ...
    let mut lines = vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            "Enter the path to the CODEX_HOME directory:",
            Style::default().fg(t.text),
        )),
        Line::from(Span::styled(
            "(Leave empty for default ~/.codex)",
            Style::default().fg(t.text_muted),
        )),
        Line::from(""),
        Line::from(vec![
...
/// Render the Codex config delete confirmation dialog
pub fn render_codex_config_delete_dialog(
    frame: &mut Frame,
    area: Rect,
    config: &CodexConfig,
...
/// Render the Codex config selector overlay
pub fn render_codex_config_selector(
    frame: &mut Frame,
    area: Rect,
    configs: &[CodexConfig],
    selected_index: usize,
...
mod tests {
    ...
    fn test_render_empty_state_does_not_panic() {
        // Basic sanity check
        let store = CodexConfigStore::new();
