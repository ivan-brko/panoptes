...
// ========================================================================
// Codex Session Creation Handler
// ========================================================================

/// Handle key while creating a new Codex session
pub fn handle_creating_codex_session_key(app: &mut App, key: KeyEvent) -> Result<()> {
    if key.kind != KeyEventKind::Press {
        return Ok(());
    }
    match key.code {
        ...
        KeyCode::Enter => {
            // Check if we need to show Codex config selector
            ...
            if config_count > 1 {
                ...
                let config = app.codex_config_store.configs_sorted()[0].clone();
                create_codex_session_with_config(app, config)?;
            } else {
                create_codex_session_without_config(app)?;
            }
        }
        KeyCode::Backspace => {
            app.state.new_session_name.pop();
...
/// Handle key while selecting Codex config
pub fn handle_selecting_codex_config_key(app: &mut App, key: KeyEvent) -> Result<()> {
    if key.kind != KeyEventKind::Press {
        return Ok(());
    }

    let config_count = app.state.available_codex_configs.len();
...
fn create_codex_session_without_config(app: &mut App) -> Result<()> {
    ...
    match app.sessions.create_codex_session(
        name.clone(),
        working_dir,
        project_id,
        branch_id,
...
/// Create a Codex session with the selected Codex config
fn create_codex_session_with_config(
    app: &mut App,
    ...
    match app.sessions.create_codex_session_with_config(
        name.clone(),
        working_dir,
        project_id,
        branch_id,
        ...
        Ok(session_id) => {
            tracing::info!(
                "Created Codex session: {} with config: {}",
                name,
                config.name
            );
    ...
    app.state.creating_session_codex_config = None;
    Ok(())
}

// ========================================================================
// Codex Config Input Handlers
// ========================================================================

/// Maximum length for Codex config name
const MAX_CODEX_CONFIG_NAME_LEN: usize = 50;
/// Maximum length for Codex config path
const MAX_CODEX_CONFIG_PATH_LEN: usize = 500;

/// Handle key while entering Codex config name (step 1)
pub fn handle_adding_codex_config_name_key(app: &mut App, key: KeyEvent) -> Result<()> {
    if key.kind != KeyEventKind::Press {
        return Ok(());
    }
    match key.code {
        ...
        KeyCode::Enter => {
            let name = app.state.new_codex_config_name.trim().to_string();
            if name.is_empty() {
                app.state.error_message = Some("Config name cannot be empty".to_string());
                return Ok(());
            }
            if app.codex_config_store.find_by_name(&name).is_some() {
                app.state.error_message = Some(format!("Config '{}' already exists", name));
                return Ok(());
            }
            app.state.input_mode = InputMode::AddingCodexConfigPath;
            app.state.new_codex_config_path.clear();
            clear_path_completions(app);
        }
        KeyCode::Backspace => {
            app.state.new_codex_config_name.pop();
        }
        KeyCode::Char(c) => {
            if app.state.new_codex_config_name.len() < MAX_CODEX_CONFIG_NAME_LEN {
                app.state.new_codex_config_name.push(c);
            }
        }
        _ => {}
    }
...
/// Handle key while entering Codex config path (step 2)
pub fn handle_adding_codex_config_path_key(app: &mut App, key: KeyEvent) -> Result<()> {
    if key.kind != KeyEventKind::Press {
        return Ok(());
    }
    match key.code {
        KeyCode::Esc => {
            if app.state.show_path_completions {
                clear_codex_config_path_completions(app);
            } else {
                app.state.input_mode = InputMode::Normal;
                app.state.new_codex_config_name.clear();
                app.state.new_codex_config_path.clear();
        ...
        KeyCode::Tab => {
            if app.state.show_path_completions && !app.state.path_completions.is_empty() {
                apply_codex_config_path_completion(app);
            } else {
                update_codex_config_path_completions(app);
            }
        }
        KeyCode::BackTab | KeyCode::Up => {
            if app.state.show_path_completions {
        ...
        KeyCode::Enter => {
            ...
            let name = std::mem::take(&mut app.state.new_codex_config_name);
            let path_str = std::mem::take(&mut app.state.new_codex_config_path);

            // Empty path means default config
            let codex_home = if path_str.trim().is_empty() {
                None
            } else {
                let expanded = PathBuf::from(shellexpand::tilde(&path_str).into_owned());
                if !expanded.is_dir() {
                    app.state.error_message =
                        Some(format!("Directory does not exist: {}", expanded.display()));
                    app.state.new_codex_config_name = name;
                    app.state.new_codex_config_path = path_str;
                    return Ok(());
                }
                Some(expanded)
            };
            ...
            if app
                .codex_config_store
                .is_codex_home_used(codex_home.as_deref())
            {
                let path_display = codex_home
                    .as_ref()
                    .map(|p| p.display().to_string())
                    .unwrap_or_else(|| "default".to_string());
                app.state.error_message = Some(format!("Path already used: {}", path_display));
            ...
            // Create the config
            let config = crate::codex_config::CodexConfig::new(name.clone(), codex_home);
            app.codex_config_store.add(config);
            ...
            tracing::info!("Added Codex config: {}", name);
            app.state.input_mode = InputMode::Normal;
        }
        KeyCode::Backspace => {
            app.state.new_codex_config_path.pop();
            update_codex_config_path_completions(app);
        }
        KeyCode::Char(c) => {
            if app.state.new_codex_config_path.len() < MAX_CODEX_CONFIG_PATH_LEN {
                app.state.new_codex_config_path.push(c);
                update_codex_config_path_completions(app);
            }
        }
        _ => {}
...
// ========================================================================
// Codex Config Path Completion Helpers
// ========================================================================

fn update_codex_config_path_completions(app: &mut App) {
    let completions = crate::path_complete::get_completions(&app.state.new_codex_config_path);
    app.state.path_completions = completions;
    app.state.path_completion_index = 0;
    app.state.show_path_completions = !app.state.path_completions.is_empty();
}

fn clear_codex_config_path_completions(app: &mut App) {
    app.state.path_completions.clear();
    app.state.path_completion_index = 0;
    app.state.show_path_completions = false;
}

fn apply_codex_config_path_completion(app: &mut App) {
    if let Some(path) = app
        .state
        .path_completions
        .get(app.state.path_completion_index)
    {
        app.state.new_codex_config_path = crate::path_complete::path_to_input(path);
        update_codex_config_path_completions(app);
    }
}
