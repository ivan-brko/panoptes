...
- `app/selection.rs` - Mouse text selection: normalization, click counting, word/line expansion, and the stream/block `Shape` (pure; routed from `app/mod.rs`, painted by `tui/views/session.rs`)
